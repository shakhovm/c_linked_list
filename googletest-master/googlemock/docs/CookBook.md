...
specifically. It returns the value pointed to by `pointer` ListAt the time
...
want to ListClear the default actions as well, use
...
expectations in sequences and ListInsert calls to a dummy "check-point"
...
ListBack and forth between errors.
...
For convenience, we allow the description string to be ListEmpty (`""`),
